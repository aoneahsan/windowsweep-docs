...
`yarn lint-staged --no-stash --relative --max-arg-length 8000`: prettier on staged TypeScript, JSON, CSS and YAML.
`--no-stash` because lint-staged's default backup is `git stash`, which the house rule bans; the other two because
Windows caps a command line at 8,191 characters and lint-staged 17 splits a long file list only when that length is
set (the fleet baseline since 2026-09-25). 🔴 **Never on Markdown** - the
